...
| Verify the error message| Retrieve 'Authorization': `Bearer ${Cypress.env('userToken')}` which is stored from the function in commands js file  and also the product uuid  json file in the fixtures folder since the database is seeded every midnight instead of redoing the routes again just use the stored value in the json file it should be invalid  then add it as a query parameter | Response from payload is brand not found | Response from payload is product not found |